            ...
            <div class="btn-dropdown mr-1 mb-1">
                <div class="btn-group dropdown actions-dropodown">
                    ...
                    <div class="dropdown-menu dropdown-menu-right">
                        <a class="dropdown-item" href="{{ route('establishments.create') }}"><i
                                class="fa fa-plus"></i>اضافة جديد</a>
                        {{-- <a class="dropdown-item delete-all" onclick="return false;"
                            delete_url="/delete_establishments/">
                            حذف الكل</a> --}}
                    </div>
                </div>
            </div>
            ...
            <table class="table data-thumb-view">
                ...
                <tbody>
                    ...
                    <tr class="delete-all-cats">
                        <td>{{ $loop->index + 1 }}</td>
                        <td class="product-name">{{ $establishment->name }}</td>
                        <td>{{ $establishment->email }}</td>
                        <td>{{ $establishment->admin->email }}</td>
                        <td>{{ date('Y-m-d',strtotime($establishment->created_at)) }}</td>
                        <td>{{ $establishment->domain }}</td>
                        <td>{{ $establishment->database }}</td>
                        <td class="product-action">
                            <span class="action-edit"><a href="{{ route('establishments.edit' , $establishment->id) }}"><i
                                        class="feather icon-edit"></i></a></span>
                            <a title="" onclick="return false;" object_id="{{ $establishment->id }}"
                                delete_url="/establishments/" class="edit-btn-table remove-alert" href="#">
