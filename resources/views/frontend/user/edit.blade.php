...
@section('frontend-head')
    ...
    <link href="{{ asset('frontend') }}/summernote.min.css" rel="stylesheet">
    <!-- END: Vendor CSS-->
    <!-- BEGIN: Theme CSS-->
            ...
            <div class="col-12">
                <div class="card">
                    <div class="card-header">
                        <h4 class="card-title">تعديل معلومات الدخول لـ {{ $user->name }}</h4>
                    </div>
                    <div class="card-content">
                        <div class="card-body">
                            <form method="post" action="{{ route('update_pro') }}" enctype="multipart/form-data" id="myformedit">
                                @csrf
                                {{ method_field('PATCH') }}
                                <div class="form-row">
                                    <div class="col-md-12 col-12 mb-3">
                                        <label for="email">البريد لتسجيل الدخول</label>
                                        <input type="email" name="email" class="form-control" id="email" placeholder="البريد لتسجيل الدخول" value="{{ $user->email }}" required>
                                        @error('email')
                                    ...
                                    <div class="col-md-12 col-12 mb-3">
                                        ...
                                        <input type="tel" name="phone" class="form-control" id="phone" placeholder="الهاتف" value="{{ $user->phone }}" required>
                                        @error('phone')
                                        <span class="text-danger">{{ $message }}</span>
                                        @enderror
                                    </div>
                                    <div class="col-md-12 col-12 mb-3">
                                        <label for="password">الرقم السري ( يتم الكتابة في حالة التغيير فقط )</label>
                                        <input type="password" name="password" class="form-control" placeholder="الرقم السري ( يتم الكتابة في حالة التغيير فقط )" value="">
                                        @error('password')
                                        <span class="text-danger">{{ $message }}</span>
                                        @enderror
                                    </div>
...
@section('frontend-footer')
    ...
    <script src="{{ asset('frontend') }}/summernote.min.js"></script>
    <script>
